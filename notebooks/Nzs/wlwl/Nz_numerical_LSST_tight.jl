...
@everywhere @model function model(data;
    ...
    for i in 1:n
        DESwl__0_nz[i] ~ TruncatedNormal(nz_k0[i], 0.2 .* sqrt.(diag(cov_k0))[i], -0.07, 0.5) 
        DESwl__1_nz[i] ~ TruncatedNormal(nz_k1[i], 0.2 .* sqrt.(diag(cov_k1))[i], -0.07, 0.5) 
        DESwl__2_nz[i] ~ TruncatedNormal(nz_k2[i], 0.2 .* sqrt.(diag(cov_k2))[i], -0.07, 0.5) 
        DESwl__3_nz[i] ~ TruncatedNormal(nz_k3[i], 0.2 .* sqrt.(diag(cov_k3))[i], -0.07, 0.5) 
    end
...
folname = string("Nzs40_LSST_numerical_lite_tight_", "TAP_", TAP)
